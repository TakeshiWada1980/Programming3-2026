...
- src/prac00.ts の **第07行目** の `const x: number = 10;` を `const x: number = "10";` に書き換えてください。
  ...
  - ただし、**実行できることと、計算結果が意図どおりであることは別**です。合計は `30` と `1020` のどちらになったでしょうか。<span class="masked">文字列の `"10"` と数値の `20` に `+` を使うと、ここでは文字列の連結になり `1020` となります。`: number` と書いても、実行時に `"10"` が数値の `10` へ自動変換されるわけではありません。</span>
