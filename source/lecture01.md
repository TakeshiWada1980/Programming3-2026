...
選択肢問題では、正解を選ぶだけではなく「他の選択肢は、どこが違うのか」も説明できることが理想です。
...
::: {.balloon .char-01 .face-02 .tone-yellow}
この授業で目指してほしいのは、**単に自分の手でアプリを実装できるようになることではありません**。実際にアプリを設計・実装し、テストし、修正するという実経験を通して <span class="masked">「どのような仕組みで実現されているのか」や「どこで、どのような問題が起こりうるのか」</span> を理解できるようになることです。

実際に、手を動かして実装する経験をしてみると、正常に動く場合だけでなく、**入力が不正だったらどうなるのか**、**通信や保存に失敗したらどうするのか**、**仕様を変更したときにどこへ影響が及ぶのか**、といった問題が見えてきます。こうした問題や例外を知っているからこそ、AI に対しても、単に「XXを作って」と依頼するのではなく、「この場合はこう扱う」「ここではこの問題が想定されるのでこうして」「この条件を満たしていることをテストする」といった、異常系に対する各種想定を含んだ設計を与えられるようになります。
:::

::: {.balloon .char-01 .face-01 .tone-pink}
重要なのは、コードを書くことそのものではなく、**実装を経験したからこそ見えてくる「起こりうる問題」や「判断すべきこと」を、仕様や設計条件として言語化してAIに与える設計力** です。そして、AIが提案した実装を理解し、テストし、評価し、必要に応じて改善できることです。

この授業では「**プログラミング**」を単なる実装技能としてではなく、**<u>ソフトウェアを設計し、判断し、その成果に責任を持つための基礎となる力</u>** として身につけてほしいと考えています。
:::

### 成績評価法

**総合成績**（学年末の最終成績）は、次のように評価します。[シラバス](https://www.ct.omu.ac.jp/studies/classes/syllabus-reg/) からの抜粋です。
...
> TypeScript の実行・開発環境を構築しているのですが、`npm init -y` というコマンドは、なんのために実行するのですか？また、実行すると、プロジェクトフォルダにどのような影響を与えますか？

> TypeScript の実行・開発環境を構築しているのですが、`npm i -D --save-exact typescript@6.0.3 @types/node@24.13.6` というコマンドは、なんのために実行するのですか？また、実行すると、プロジェクトフォルダにどのような影響を与えますか？
...
- 参考: [YouTube検索「ソフトウェアテスト」](https://www.youtube.com/results?search_query=ソフトウェアテスト)
...
> ソフトウェアテストって何のために必要なんですか。プログラムを見れば分かるのに、わざわざテストコード書いて面倒なだけだと思います。しかも、プログラムを変更するたびにテストコードも併せて書き直さないといけないし。
...
::: {.balloon .char-01 .face-02 .tone-yellow}
実行環境によっては、ログレベル (log, error, warn...) に応じて「**色分け**」や「**装飾付き**」で出力されたり、フィルタリング機能 (＝特定のレベル以上のメッセージのみを表示する機能) が提供されます。

でも、まずは細かな使い分けは考えずに `console.log()` だけの使用 (もしくは `console.error()` との2つの使い分け) で十分です。
:::
...
- 参考: [YouTube検索「テスト駆動開発」](https://www.youtube.com/results?search_query=テスト駆動開発)
...
- 本科目は「**学修単位科目**」です。各回の講義資料は、4時間相当の授業時間外学習を含めて **標準的な学生が学習に「6時間」を要する内容** で構成しています。疑問点や不明点などを「生成AI」で調べて深掘りしたり、C言語やPythonプログラムを TypeScript に移植したりすることをお勧めします。
...
- 次回の授業のはじめに「**小テスト**」を実施します。筆記用具を忘れず持参してください。
